  ...
  <div class="p-4">
    ...
    <div v-else class="space-y-4">
      <div class="bg-white rounded-xl shadow-lg border border-gray-200 overflow-hidden mt-4">
        <BaseTable
          empty-title="Laporan Kesekretariatan Tidak Ditemukan"
      ...
      >
        <template #custom-actions>
          ...
          <select
            ...
            class="w-full sm:w-64 rounded-lg border-gray-300 shadow-sm px-3 py-2 text-gray-700 focus:border-green-900 focus:ring-2 focus:ring-green-900 transition ml-2"
          >
        ...
        <template #cell-tanggal="{ row }">{{ row[1] }}</template>
        <template #cell-uraian="{ row }">{{ row[2] }}</template>
        <template #cell-nik="{ row }">{{ row[3] }}</template>
        <template #cell-alamat="{ row }">{{ row[4] }}</template>
        <template #cell-kecamatan="{ row }">{{ row[5] }}</template>
        <template #cell-kode_akun="{ row }">{{ row[6] }}</template>
        <template #cell-kredit="{ row }">{{ row[7] }}</template>
      </BaseTable>
      </div>
    </div>
