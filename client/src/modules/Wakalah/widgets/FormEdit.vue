...
import { edit_wakalah, get_info_edit_wakalah } from '@/service/wakalah';
...
const form = ref<{ nama: string; nik: string; kecamatan_id: number | null; desa_id: number | null }>({
  nama: '',
  nik: '',
  kecamatan_id: null,
  desa_id: null,
...
watch(() => form.value.kecamatan_id, async (newVal, oldVal) => {
  if (oldVal !== null && newVal !== oldVal && !isLoading.value) {
    form.value.desa_id = null;
  }
  await fetchDesa(newVal);
});

const fetchData = async () => {
  if (!props.selectedWakalah || !props.selectedWakalah.id) return;
  isLoading.value = true;
  try {
    await fetchKecamatan();
    const response = await get_info_edit_wakalah(props.selectedWakalah.id);
    form.value.nama = response.data.nama;
    form.value.nik = response.data.nik;
    form.value.kecamatan_id = response.data.kecamatan_id;
    await fetchDesa(response.data.kecamatan_id);
...
const handleSubmit = async () => {
  ...
  try {
    const payload = {
      id: props.selectedWakalah.id,
      nama: form.value.nama,
      nik: form.value.nik,
      desa_id: form.value.desa_id
    };
    const response = await edit_wakalah(payload);
...
watch(() => props.selectedWakalah, (val) => {
  if (props.isModalOpen && val?.id) fetchData();
});
watch(() => props.isModalOpen, (val) => {
  if (val && props.selectedWakalah?.id) fetchData();
});
</script>
    ...
    <div v-if="isModalOpen" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm" role="dialog">
      ...
      <div v-else class="relative max-w-md w-full bg-white shadow-2xl rounded-2xl p-6 space-y-6">
        <div class="flex items-center justify-between">
          <h2 class="text-xl font-bold text-gray-800">Edit Wakalah</h2>
